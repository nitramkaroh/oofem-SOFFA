...
EngngModel ::EngngModel( int i, EngngModel *_master ) :
    domainNeqs(), domainPrescribedNeqs(),
...
int EngngModel ::forceEquationNumbering()
{
...
void EngngModel ::updateAttributes( MetaStep *mStep )
{
...
void EngngModel ::saveStepContext( TimeStep *tStep, ContextMode mode )
{
  if ( this->giveContextOutputMode() == COM_Always || this->giveContextOutputMode() == COM_Required || ( this->giveContextOutputMode() == COM_UserDefined && tStep->giveNumber() % this->giveContextOutputStep() == 0 ) ) {
...
void EngngModel ::assembleExtrapolatedForces( FloatArray &answer, TimeStep *tStep, CharType type, Domain *domain )
{
...
void EngngModel ::assemblePrescribedExtrapolatedForces( FloatArray &answer, TimeStep *tStep, CharType type, Domain *domain )
{
  // Simply assembles contributions from each element in domain
  IntArray loc;
  FloatArray charVec, delta_u;
...
void EngngModel ::assembleVectorFromContacts( FloatArray &answer, TimeStep *tStep, CharType type, ValueModeType mode,
    const UnknownNumberingScheme &s, Domain *domain, FloatArray *eNorms )
{
  if ( domain->hasContactManager() ) {
...
void EngngModel ::updateComponent( TimeStep *tStep, NumericalCmpn cmpn, Domain *d )
{
  OOFEM_ERROR( "Unknown Type of component." );
}


void EngngModel ::updateSolution( FloatArray &solutionVector, TimeStep *tStep, Domain *d )
{
  OOFEM_ERROR( "updateSolution is not implemented." );
}


void EngngModel ::updateInternalRHS( FloatArray &answer, TimeStep *tStep, Domain *d, FloatArray *eNorms )
{
  OOFEM_ERROR( "updateInternalRHS is not implemented." );
}


void EngngModel ::updateMatrix( SparseMtrx &mat, TimeStep *tStep, Domain *d )
{
  OOFEM_ERROR( "updateMatrix is not implemented." );
}


void EngngModel ::initStepIncrements()
//
...
void EngngModel ::updateDomainLinks()
{
...
void EngngModel ::setDomain( int i, Domain *ptr, bool iDeallocateOld )
{
...
void EngngModel ::initParallelContexts()
{
...
void EngngModel ::letOutputBaseFileNameBe( const std ::string &src )
{
...
void EngngModel ::giveAnalysisTime( int &rhrs, int &rmin, int &rsec, int &uhrs, int &umin, int &usec )
{
...
void EngngModel ::terminateAnalysis()
{
...
int EngngModel ::checkProblemConsistency()
{
  int result = 1;
...
void EngngModel ::postInitialize()
{
...
void EngngModel ::init()
{
  initModuleManager.doInit();
}


void EngngModel ::initParallel()
{
#ifdef __USE_MPI
  int len;
...
void EngngModel ::drawYourself( oofegGraphicContext &gc )
{
  ...
  if ( ( plotMode == OGC_nodeAnnotation ) || ( plotMode == OGC_nodeGeometry ) || ( plotMode == OGC_essentialBC ) || ( plotMode == OGC_naturalBC ) || ( plotMode == OGC_nodeScalarPlot ) || ( plotMode == OGC_nodeVectorPlot ) ) {
    this->drawNodes( gc );
  } else {
...
void EngngModel ::initializeCommMaps( bool forceInit )
{
#ifdef __PARALLEL_MODE
...
int EngngModel ::updateSharedDofManagers( FloatArray &answer, const UnknownNumberingScheme &s, int ExchangeTag )
{
  if ( isParallel() ) {
#ifdef __PARALLEL_MODE
    int result = 1;
...
int EngngModel ::exchangeRemoteElementData( int ExchangeTag )
{
...
#ifdef __PARALLEL_MODE
void EngngModel ::balanceLoad( TimeStep *tStep )
{
  ...
  if ( tStep->isNotTheLastStep() ) {
    ...
    if ( ( _d == LoadBalancerMonitor ::LBD_RECOVER ) || ( ( tStep->isTheFirstStep() ) && force_load_rebalance_in_first_step ) ) {
...
int EngngModel ::packRemoteElementData( ProcessCommunicator &processComm )
{
  int result = 1;
...
int EngngModel ::unpackRemoteElementData( ProcessCommunicator &processComm )
{
  int result = 1;
...
int EngngModel ::packDofManagers( ArrayWithNumbering *srcData, ProcessCommunicator &processComm )
{
...
int EngngModel ::unpackDofManagers( ArrayWithNumbering *destData, ProcessCommunicator &processComm )
{
