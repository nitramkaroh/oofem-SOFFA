...
#include "Tensor5_contracted.hpp"
...
namespace FTensor
{
  ...
  {
  protected:
    T data[Tensor_Dim0][Tensor_Dim1][Tensor_Dim2][Tensor_Dim3][Tensor_Dim4];

  public:
    /* Initialization operators */
    template <class... U> Tensor5(U... d) : data{d...}
    {
    ...
    T &operator()( const int N1, const int N2, const int N3, const int N4, const int N5 )
    {
      ...
      if(N1 >= Tensor_Dim0 || N1 < 0 || N2 >= Tensor_Dim1 || N2 < 0
        ...
        {
          std::stringstream s;
          s << "Bad index in Tensor5<T," << Tensor_Dim0 << "," << Tensor_Dim1
            << "," << Tensor_Dim2 << "," << Tensor_Dim3 << "," << Tensor_Dim4 << ">.operator(" << N1
            << "," << N2 << "," << N3 << "," << N4 "," << N5 << ") const" << std::endl;
          throw std::out_of_range(s.str());
        }
#endif
      return data[N1][N2][N3][N4][N5];
    }
    ...
    template <char i, char j, char k, char l, char m, int Dim0, int Dim1, int Dim2,
              int Dim3, int Dim4>
    typename std::enable_if<
      (Tensor_Dim0 >= Dim0 && Tensor_Dim1 >= Dim1 && Tensor_Dim2 >= Dim2
       && Tensor_Dim3 >= Dim3 && Tensor_Dim4 >= Dim4),
      Tensor5_Expr<Tensor5<T, Tensor_Dim0, Tensor_Dim1, Tensor_Dim2, Tensor_Dim3, Tensor_Dim4>,
                   T, Dim0, Dim1, Dim2, Dim3, Dim4, i, j, k, l, m>>::type
    operator()(const Index<i, Dim0>, const Index<j, Dim1>,
               const Index<k, Dim2>, const Index<l, Dim3>, const Index<m, Dim4>)
    {
      return Tensor5_Expr<
        Tensor5<T, Tensor_Dim0, Tensor_Dim1, Tensor_Dim2, Tensor_Dim3, Tensor_Dim4>, T,
        Dim0, Dim1, Dim2, Dim3, Dim4, i, j, k, l, m>(*this);
    };
    ...
    typename std::enable_if<
      (Tensor_Dim0 >= Dim0 && Tensor_Dim1 >= Dim1 && Tensor_Dim2 >= Dim2
       && Tensor_Dim3 >= Dim3 && Tensor_Dim4 >= Dim4),
      Tensor5_Expr<
        const Tensor5<T, Tensor_Dim0, Tensor_Dim1, Tensor_Dim2, Tensor_Dim3, Tensor_Dim4>,
        T, Dim0, Dim1, Dim2, Dim3, Dim4, i, j, k, l, m>>::type
    operator()(const Index<i, Dim0>, const Index<j, Dim1>,
        const Index<k, Dim2>, const Index<l, Dim3>, const Index<m, Dim4> ) const
    {
      return Tensor5_Expr<
        const Tensor5<T, Tensor_Dim0, Tensor_Dim1, Tensor_Dim2, Tensor_Dim3, Tensor_Dim4>,
        T, Dim0, Dim1, Dim2, Dim3, Dim4, i, j, k, l, m>(*this);
    };

    /* These operators are for internal contractions, resulting in a Tensor3.
     * For example something like A(i,i,k,l,m) */
...
namespace FTensor
{
  template <class T, int Tensor_Dim0, int Tensor_Dim1, int Tensor_Dim2,
            int Tensor_Dim3, int Tensor_Dim4>
  std::ostream &Tensor5_00001(
    std::ostream &os,
    const Tensor5<T, Tensor_Dim0, Tensor_Dim1, Tensor_Dim2, Tensor_Dim3, Tensor_Dim4> &t,
    const int &iterator0, const int &iterator1, const int &iterator2, const int &iterator3)
  {
    os << '[';
    for(int i = 0; i < Tensor_Dim4 - 1; ++i)
      {
        os << t(iterator0, iterator1, iterator2, iterator3, i);
        os << ',';
      }
    os << t(iterator0, iterator1, iterator2, iterator3, Tensor_Dim4 - 1);
    os << ']';
    ...
    const int &iterator0, const int &iterator1, const int &iterator2)
  {
    ...
    for(int i = 0; i < Tensor_Dim3 - 1; ++i)
      {
        FTensor::Tensor5_00001(os, t, iterator0, iterator1, iterator2, i);
        os << ',';
      }
    FTensor::Tensor5_00001(os, t, iterator0, iterator1, iterator2, Tensor_Dim3 - 1);
    os << ']';
    ...
    const int &iterator0, const int &iterator1)
  {
    ...
    for(int i = 0; i < Tensor_Dim2 - 1; ++i)
      {
        FTensor::Tensor5_00010(os, t, iterator0, iterator1, i);
        os << ',';
      }
    FTensor::Tensor5_00010(os, t, iterator0, iterator1, Tensor_Dim2 - 1);
    os << ']';
    ...
    const int &iterator0 )
{
  ...
  for ( int i = 0; i < Tensor_Dim1 - 1; ++i ) {
    FTensor::Tensor5_00100( os, t, iterator0, i );
    os << ',';
  }
  FTensor::Tensor5_00100( os, t, iterator0, Tensor_Dim1 - 1 );
  os << ']';
...
std::ostream &operator<<(std::ostream &os,
                         const FTensor::Tensor5<T, Tensor_Dim0, Tensor_Dim1,
                                                Tensor_Dim2, Tensor_Dim3, Tensor_Dim4> &t)
{
  os << '[';
  for(int i = 0; i < Tensor_Dim0 - 1; ++i)
    {
      FTensor::Tensor5_01000(os, t, i);
      os << ',';
    }
  FTensor::Tensor5_01000(os, t, Tensor_Dim0 - 1);
  os << ']';
