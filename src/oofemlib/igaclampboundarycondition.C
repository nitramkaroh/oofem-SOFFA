...
#include "igaclampboundarycondition.h"
#include "set.h"
#include "domain.h"
#include "node.h"
#include "masterdof.h"
...
namespace oofem {
REGISTER_BoundaryCondition( IGAClampBoundaryCondition );


IGAClampBoundaryCondition ::IGAClampBoundaryCondition( int n, Domain *d ) :
    ActiveBoundaryCondition( n, d )
{
}
//IGAClampBoundaryCondition ::IGAClampBoundaryCondition( int n, Domain *d ) :
//    BoundaryCondition( n, d )
//{
//}


void IGAClampBoundaryCondition ::initializeFrom( InputRecord &ir )
{
    ActiveBoundaryCondition ::initializeFrom( ir );
    //BoundaryCondition ::initializeFrom( ir );
    IR_GIVE_FIELD( ir, this->elementNumber, _IFT_IGAClampBoundaryCondition_ElementNumber );
    double lp;
    IR_GIVE_FIELD( ir, lp, _IFT_IGAClampBoundaryCondition_LocalPosition );
    this->localPosition.resize( 1 );
    this->localPosition.at(1) = lp;

    int ks;
    IR_GIVE_FIELD( ir, ks, _IFT_IGAClampBoundaryCondition_BcKnotSpan );
    this->BcKnotSpan.resize( 1 );
    this->BcKnotSpan.at( 1 ) = ks;

    IR_GIVE_OPTIONAL_FIELD( ir, this->penalty, _IFT_IGAClampBoundaryCondition_p );
...
void IGAClampBoundaryCondition ::assemble( SparseMtrx &answer, TimeStep *tStep,
    ...
    void *lock )
{
    ...
    // compute normal
    FloatArray normal;
    interp->boundaryEvalNormal( normal, 1, this->localPosition, FEIIGAElementGeometryWrapper( el, &this->BcKnotSpan ) );
...
void IGAClampBoundaryCondition ::assembleVector( FloatArray &answer, TimeStep *tStep,
    ...
    void *lock )
{
    ...
    if ( type == InternalForcesVector ) {
        ...
        FloatMatrix dNdNt;
        dNdNt.beDyadicProductOf( dNdxi, dNdxi );
        FloatArray prod, prod2;
        prod.beProductOf( dNdNt, ur );
        prod2 = dNdxi.dotProduct( ur ) * dNdxi;

        //( prod2 - prod ).printYourself();

        // assemble to the element vector
        FloatArray answerTemp;
        int ndofs = elem->computeNumberOfDofs();
        answerTemp.resize( ndofs );
        answerTemp.assemble( prod2, lc );

        //answerTemp.assemble( this->penalty*dNdxi.dotProduct( ur ) * dNdxi, lc );


        // assemble to the global vector
        //EModelDefaultEquationNumbering dn;
        elem->giveLocationArray( lcGlob, s );
        answer.assemble( this->penalty * answerTemp, lcGlob );
    }
}

int IGAClampBoundaryCondition ::giveLocalCodeNumbers( IntArray &answer, Element *elem, IntArray &span )
{
    int nsd;
...
//
//
//void IGAClampBoundaryCondition ::giveLagrangianMultiplierLocationArray( const UnknownNumberingScheme &r_s, std ::vector<IntArray> &answer )
//{
//    int size = this->masterSet.giveSize();
//    answer.resize( size );
//    IntArray dofIdArray = {
...
//
//
//void IGAClampBoundaryCondition ::giveLocationArrays( std ::vector<IntArray> &rows, std ::vector<IntArray> &cols, CharType type, const UnknownNumberingScheme &r_s, const UnknownNumberingScheme &c_s )
//{
