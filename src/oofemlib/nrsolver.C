...
void NRSolver ::initializeFrom( InputRecord &ir )
{
...
void NRSolver ::initPrescribedEqs()
{
...
void NRSolver ::applyConstraintsToStiffness( SparseMtrx &k )
{
...
void NRSolver ::applyConstraintsToLoadIncrement( int nite, const SparseMtrx &k, FloatArray &R,
    referenceLoadInputModeType rlm, TimeStep *tStep )
{
  ...
  if ( ( rlm == rlm_total ) && ( !tStep->isTheFirstStep() ) ) {
    ...
    factor -= engngModel->giveDomain( 1 )->giveFunction( prescribedDisplacementTF )->evaluateAtTime( tStep->giveTargetTime() - tStep->giveTimeIncrement() );
  }
...
void NRSolver ::printState( FILE *outputStream )
{
#ifdef VERBOSE
  // print quasi reactions if direct displacement control used
...
bool NRSolver ::checkConvergence( FloatArray &RT, FloatArray &F, FloatArray &rhs, FloatArray &ddX, FloatArray &X,
    ...
    for ( int dg = 1; dg <= nccdg; dg++ ) {
      ...
      if ( rtolf.at( 1 ) > 0.0 ) {
        ...
        if ( dg_forceScale.find( dg ) != dg_forceScale.end() ) {
          forceErr = sqrt( dg_forceErr.at( dg ) / ( dg_totalLoadLevel.at( dg ) + internalForcesEBENorm.at( dg ) + idsInUse.at( dg ) * dg_forceScale[dg] * dg_forceScale[dg] ) );
