...
#include "hardmagneticbodyload.h"
...
namespace oofem {
REGISTER_BoundaryCondition( HardMagneticBodyLoad );

    void HardMagneticBodyLoad::initializeFrom( InputRecord &ir )
    {
        ActiveBoundaryCondition::initializeFrom( ir );

	    FloatArray b_temp, m_temp;
        IR_GIVE_FIELD( ir, b_temp, _IFT_HardMagneticBodyLoad_b_ext );
	    b_app = FloatArrayF< 3 >( b_temp );

        IR_GIVE_FIELD( ir, m_temp, _IFT_HardMagneticBodyLoad_mjump );
        m_jump = FloatArrayF<3>( m_temp );

        IR_GIVE_FIELD( ir, ltf_index, _IFT_HardMagneticBodyLoad_ltf );

        mu0 = BASE_VACUUM_PERMEABILITY_MU_0;
        IR_GIVE_OPTIONAL_FIELD( ir, mu0, _IFT_HardMagneticBodyLoad_mu_0 );
    }

    void HardMagneticBodyLoad::assemble( SparseMtrx &answer, TimeStep *tStep,
    ...
    void HardMagneticBodyLoad::assembleVector(FloatArray& answer , TimeStep* tStep ,
    ...
    void HardMagneticBodyLoad::giveLocationArrays( std::vector<IntArray> &rows, std::vector<IntArray> &cols, CharType type, const UnknownNumberingScheme &r_s, const UnknownNumberingScheme &c_s )
    {
        if ( type != TangentStiffnessMatrix ) {
            return;
        }
    ...
    void HardMagneticBodyLoad::computeLoadVectorFromElement( FloatArray &answer, Element *e, int iSurf, TimeStep *tStep, ValueModeType mode )
    {
	
        
    }

    void HardMagneticBodyLoad::computeTangentFromElement( FloatMatrix &answer, Element *e, int iSurf, TimeStep *tStep )
    {
    ...
    void HardMagneticBodyLoad::computeNumericalTangentFromElement( FloatMatrix &answer, Element *e, int side, TimeStep *tStep, double perturb )
    {
    ...
    void HardMagneticBodyLoad::computePerturbedLoadVectorFromElement( FloatArray &answer, Element *e, int iSurf, TimeStep *tStep, double perturb, int index )
    {
