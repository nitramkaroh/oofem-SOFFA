...
VarBasedDamageMaterial :: initializeFrom(InputRecord &ir)
{
  ...
  if (this->phaseFieldModelType == phaseFieldModel_Wu) {
    ...
    IsotropicLinearElasticMaterial *isole = dynamic_cast< IsotropicLinearElasticMaterial * >(this->linearElasticMaterial);
    this->a1 = 4.*isole->giveYoungsModulus()*this->gfInf/pow(this->ft,2);
    //  
  ...
  if (this->phaseFieldModelType == phaseFieldModel_JZ){
    ...
    if ( equivStrainTypeRecord == 0 ) {
        ...
        throw ValueInputException(ir, _IFT_VarBasedDamageMaterial_equivstraintype, "Unknown equivStrainType");
    }
...
double
VarBasedDamageMaterial :: computeQ_Wu(double damageDrivingVariable) const
{
  return this->a1*damageDrivingVariable + this->a2*pow(damageDrivingVariable,2) + this->a3*pow(damageDrivingVariable,3);
}

double
VarBasedDamageMaterial :: computeQ_Wu_prime1(double damageDrivingVariable) const
{
...
VarBasedDamageMaterial :: computeDamage(double &answer, double damageDrivingVariable, GaussPoint *gp)
{
  ...
  if (this->phaseFieldModelType == phaseFieldModel_JZ) { 
    ...
    answer = Q / ( Q + pow(1 - damageDrivingVariable,this->p) ); 
  } else {
    OOFEM_ERROR("Unknown phase-field model type");
  }

  answer = min(answer, maxDamage); //default value of maxDamage = 0.995
    if(answer < 0.) {
    answer = 0.;
  }
...
void
VarBasedDamageMaterial :: computeDamagePrime(double &answer, double damageDrivingVariable, GaussPoint *gp) const
{
  ...
  if (this->phaseFieldModelType == phaseFieldModel_JZ) {
    ...
    answer = (prime1_Q* (Q + pow(1.-damageDrivingVariable, p) ) - Q * (prime1_Q - p * pow(1.-damageDrivingVariable, p-1.) )) / pow((Q + pow(1.-damageDrivingVariable,p)),2);
...
double
VarBasedDamageMaterial :: solveExpLaw(double dam, double c) const
{
  double answer = 0.;
  double f, df;
  ...
  OOFEM_ERROR("No convergence in VarBasedDamageMaterial :: solveExpLaw(%g,%g)\n",dam,c);
}

double
VarBasedDamageMaterial :: compute_dissipation_Wu_prime1_in_gamma(double damageDrivingVariable) const
{
...
void
VarBasedDamageMaterial :: computeDissipationFunctionPrime(double &answer, double damage, double damageDrivingVariable,  GaussPoint *gp) const
{
...
VarBasedDamageMaterial :: computeLocalDamageDrivingVariable(double &answer, GaussPoint *gp, TimeStep *tStep)
{
  ...
  eps = this->computeEquivalentStrain(redStrain, gp, tStep);
  answer = 0.5*E*eps*eps;
  double localDamageDrivingVariable = status->giveLocalDamageDrivingVariable();
  if(answer < localDamageDrivingVariable) {
    answer = localDamageDrivingVariable;
  }
}
...
VarBasedDamageMaterial :: CreateStatus(GaussPoint *gp) const
{
    ...
    double dDiss, dDamage;
    this->computeDissipationFunctionPrime(dDiss, 0, 0, gp);
    this->computeDamagePrime(dDamage, 0, gp);
    return new VarBasedDamageMaterialStatus(gp, initialDamage, dDamage * dDiss);
}
...
VarBasedDamageMaterialStatus :: VarBasedDamageMaterialStatus(GaussPoint *g, double initialDamage, double initLocalDamageDrivingVariable) : IsotropicDamageMaterial1Status(g), GradientDamageMaterialStatusExtensionInterface()
{
  ...
  localDamageDrivingVariable = tempLocalDamageDrivingVariable = initLocalDamageDrivingVariable;
}
...
void
VarBasedDamageMaterialStatus :: saveContext(DataStream &stream, ContextMode mode)
//
...
void
VarBasedDamageMaterialStatus :: restoreContext(DataStream &stream, ContextMode mode)
//
