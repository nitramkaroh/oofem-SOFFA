...
FloatMatrixF<9, 9> MooneyRivlinHardMagnetic::compute3dMaterialStiffnessMatrix_dPdF_mit( MatResponseMode matResponseMode, GaussPoint *gp, TimeStep *tStep ) const
{
  ...
  case PBT_F:
    ...
    break;
  case PBT_R:
    vD_me = compute_numerical_stiffness_mitR( vF, gp, tStep );
    break;
  case PBT_iFt:
    OOFEM_ERROR( "Pullback type not implemented." )
    break;
  case PBT_iFtnoJ:
    ...
    D_me( i_3, j_3, k_3, l_3 ) = -J * M( q_3 ) * b( p_3 ) * Finv( q_3, p_3 ) * Finv( l_3, k_3 ) * Finv( j_3, i_3 )
...
{
  FloatMatrixF<9, 9> vD_me_num;
  FloatArrayF<9> vF_pert_fore, vF_pert_back, vP_fore, vP_back;
  double pert = 1.e-4;
  ...
  for ( int j = 1; j <= 9; j++ ) {
    ...
    // perturb forward
    vF_pert_fore = vF;
    vF_pert_fore.at( j ) = vF.at( j ) + pert;
    vP_fore = this->computeFirstPKStressVector_3d_mit(vF_pert_fore, gp, tStep);
    // perturb back
    vF_pert_back = vF;
    vF_pert_back.at( j ) = vF.at( j ) - pert;
    vP_back = this->computeFirstPKStressVector_3d_mit( vF_pert_back, gp, tStep );

    for ( int i = 1; i <= j; i++ ) { //exploit symmetry
      double val = ( vP_fore.at( i ) - vP_back.at( i ) ) / ( 2 * pert );
      vD_me_num.at(i,j) = val;
      vD_me_num.at(j,i) = val;
    }
  }
