...
#include "harmonichyperelasticmaterial.h"
...
namespace oofem {
REGISTER_Material( HarmonicHyperelasticMaterial );

HarmonicHyperelasticMaterial::HarmonicHyperelasticMaterial( int n, Domain *d ) :
    StructuralMaterial( n, d ), BaseHyperElasticMaterial()
{
}

FloatArrayF<5>
HarmonicHyperelasticMaterial::giveFirstPKStressVector_PlaneStrain( const FloatArrayF<5> &vF_ps, GaussPoint *gp, TimeStep *tStep ) const
...
FloatMatrixF<5, 5>
HarmonicHyperelasticMaterial::givePlaneStrainStiffnessMatrix_dPdF( MatResponseMode mode, GaussPoint *gp, TimeStep *tStep ) const
...
MaterialStatus *
HarmonicHyperelasticMaterial::CreateStatus( GaussPoint *gp ) const
{
  return new StructuralMaterialStatus( gp );
}


void HarmonicHyperelasticMaterial::initializeFrom( InputRecord &ir )
{
  StructuralMaterial::initializeFrom( ir );
  BaseHyperElasticMaterial::initializeFrom( ir );
  IR_GIVE_FIELD( ir, alpha, _IFT_HarmonicHyperelasticMaterial_alpha );
  IR_GIVE_FIELD( ir, beta, _IFT_HarmonicHyperelasticMaterial_beta );
  IR_GIVE_FIELD( ir, mu, _IFT_HarmonicHyperelasticMaterial_mu );
}

double HarmonicHyperelasticMaterial::compute_dFunctiondI_from_F( const Tensor2_3d &F ) const
{
  double I = this->compute_I_of_U_2d_from_F( F );
  return 1. / ( 4. * alpha ) * ( I + std::sqrt( I * I - 16 * alpha * beta ) );
}

double HarmonicHyperelasticMaterial::compute_d2FunctiondIdI_from_F( const Tensor2_3d &F ) const
{
  double I = this->compute_I_of_U_2d_from_F( F );
...
double HarmonicHyperelasticMaterial::compute_I_of_U_2d_from_F( const Tensor2_3d &F ) const
{
  return std::sqrt( F( p_3, q_3 ) * F( p_3, q_3 ) - 1. + 2. * F.compute_determinant() );
}
Tensor2_3d HarmonicHyperelasticMaterial::compute_dI_of_U_2d_dF_from_F( const Tensor2_3d &F ) const
{
  Tensor2_3d answer;
...
Tensor4_3d HarmonicHyperelasticMaterial::compute_d2I_of_U_2d_dFdF_from_F( const Tensor2_3d &F ) const
{
  Tensor4_3d answer;
  auto cofF = F.compute_cofactor();
